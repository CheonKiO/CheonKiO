...
전세사기 피해자의 75%가 2030이라는 데이터에서 출발해, 주소만 넣으면 실거래가·건축물대장·재정 안정성을 종합한 안심 리포트를 주는 서비스입니다. 처음으로 규모 있는 데이터를 다룬 첫 실전 규모의 팀 프로젝트였습니다.
...
사진 한 장과 현재 위치로 문화유산을 식별해 AI 해설을 주고, 탐방이 끝나면 퀴즈와 리포트로 복습하게 하는 서비스입니다. 국가유산청 데이터 **8,313건**을 배치로 적재하고 전량 임베딩해 Redis Stack에 인덱싱했습니다. **처음으로 AI 기반 개발을 본격적으로 시작한 프로젝트**입니다. 구현은 Claude Code로 빠르게 처리하되, AI 응답을 그대로 믿지 않고 검증한 뒤에 쓰는 방식으로 일했습니다.

프로젝트 초기 백엔드 패키지 구조와 공통 기반(`ApiResponse` 일관 응답, `GlobalExceptionHandler`, Redis·Swagger 설정)을 잡았습니다. 프론트엔드도 공통 기반 — 디자인 토큰, JWT 인터셉터(401 감지 시 refresh 후 자동 재시도), 라우터 인증 가드 — 을 세팅했습니다.

`Java 21` `Spring Boot 3.5.1` `MyBatis` `MySQL(SRID 4326)` `Redis Stack` `Jedis` `Vue 3` `TypeScript` `Kakao Maps`

### 🔍 무엇을 고민했나

#### 지저분한 원본 데이터를 서비스가 쓸 값으로

국가유산청 데이터는 시대 필드가 비어 있거나 자유 텍스트("을사년(1605)", "17세기", "백제 무왕")로 와서 그대로 쓸 수 없었습니다. 그래서 규칙을 세웠습니다 — 괄호 안 연도가 있으면 그걸 우선 쓰고, "17세기"는 중간값(1650)으로 환산하고, 연도가 아예 없으면 왕조 키워드로 삼국·고려·조선 같은 시대에 매핑합니다. 여러 연도가 섞이면 가장 이른 값을 택합니다.

삼국 668 · 고려 918 · 조선 1392처럼 시대가 갈리는 연도마다 경계값 테스트를 붙여, 667년과 668년이 서로 다른 시대로 분류되는지까지 고정했습니다.

> "8,313건을 배치로 적재했다"는 한 줄 뒤에는, 이렇게 원본을 서비스가 쓸 값으로 정규화하는 작업이 있었습니다.

#### 외부 API는 스펙대로 오지 않는다

국가유산청 이미지 API는 한 항목 안에 필드를 반복하고, 심지어 문화재 이름 자체를 XML 태그로 쓰기도 했습니다(`<대성전>`, `<권1-5>`). 이런 비표준 XML은 Jackson `XmlMapper`로 파싱하다 깨졌습니다.

그래서 파서를 이원화했습니다. 정상 XML은 선언형(`XmlMapper`)으로 두고, 태그가 깨진 응답은 정규식으로 비정상 태그를 걷어낸 뒤 StAX(`XMLStreamReader`)로 직접 순회하며 파싱합니다. 깨진 태그 케이스도 테스트로 고정해, 다음에 또 이상한 응답이 와도 어디서 걸리는지 알 수 있게 했습니다.
...
#### 같은 데이터인데 추천 조건이 정반대였던 것

추천 기능이 두 개인데, 둘 다 문화유산 임베딩 유사도를 씁니다. 그런데 거리 조건이 정반대였습니다.
...
처음엔 같은 로직을 재사용하려 했는데, "다음 여행지"로 지금 있는 동네를 추천하는 건 의미가 없다는 걸 깨닫고 나눴습니다. 같은 데이터라도 쓰이는 맥락이 다르면 반대 조건이 답이 되는 경우였습니다.
...
→ 유사도 상위 5개 추출
→ Nearest Neighbor로 이동 경로 정렬
```

"의미적으로 비슷한 곳"을 찾은 다음 "실제로 하루에 돌 수 있는 묶음"으로 만드는 게 핵심이었습니다. 클러스터링으로 지역을 좁히고, 경로는 TSP 근사(Nearest Neighbor)로 정렬하는 쪽으로 잡았습니다. 거리 계산은 Haversine으로 했습니다.
...
AI 호출에는 제한이 있어서, 비용을 최대한 아끼는 쪽으로 구조를 잡았습니다.

- **분류와 해설을 분리** — 처음엔 분류와 해설을 한 요청에 같이 담았습니다. 그런데 문화유산 식별에는 사진이 필요하지만 해설 생성에는 필요 없었습니다. 두 호출을 나누니 해설은 문화유산 단위로 공유 캐싱할 수 있게 됐고, 같은 곳을 방문한 두 번째 사용자부터는 AI 호출이 1회로 줄었습니다.
...
캐시가 채워지기 전에 같은 문화유산 요청이 동시에 들어오면 두 요청 모두 AI를 호출했습니다. 락 범위를 문화유산 ID 단위로 좁혀서, 전체 처리량은 해치지 않으면서 중복 호출만 막는 쪽으로 갔습니다.
...
- GMS 게이트웨이의 100KB 요청 제한 → 사진은 512px/JPEG 70%로 압축하고, 후보 문화유산은 이미지 대신 텍스트로 전달
- iOS HEIC는 JPEG로 변환하면 EXIF가 사라짐 → 변환 **전** 원본에서 GPS를 먼저 추출
- Spring Data Redis가 RediSearch(벡터 검색)를 지원하지 않음 → 그 부분만 Jedis로 직접 질의
...
- **API 왕복을 응답 설계로 줄임.** 마이페이지에서 완료 여행마다 썸네일을 얻으려 상세 API를 개별 호출(N+1)하던 것을, 응답 DTO에 썸네일 URL을 미리 실어 프론트의 반복 호출을 없앴습니다. 프론트에서 문제를 발견하고 백엔드 응답 설계로 고친 사례입니다.
- **AI 클라이언트를 하나로 통합.** 기능마다 각자 AI 클라이언트를 두면 재시도·파싱·예외 처리가 중복됩니다. 흩어진 걸 `GmsAiClient` 하나로 합쳐 AI 연동 지점을 단일화하는 쪽으로 갔습니다.
- **정확한 것과 유용한 것을 구분하는 감각.** 지표가 좋아도 사용자 맥락에서 무의미하면 고쳐야 한다는 걸 직접 겪었습니다.
- **커진 화면은 나눠야 한다는 것.** ReportPage 948줄, QuizPage 903줄까지 자란 컴포넌트를 각각 작은 컴포넌트로 쪼개고(ReportPage는 244줄로), 페이지는 상태 관리와 API 호출만 맡는 오케스트레이션 레이어로 역할을 좁혔습니다.
...
AI 없이 손으로 짜던 방식에서, **AI를 검증할 수 있는 범위 안에서 적극적으로 쓰는** 쪽으로 옮겨왔습니다. HisTour에서 AI를 실전에 처음 들였고, Omok 2차에서 그 위력과 한계를 동시에 겪었습니다.

지금은 Claude Code와 MCP·에이전트 도구를 일상 워크플로우에 두고 씁니다. 다만 AI를 코드를 대신 써주는 도구가 아니라, **내 판단을 되비추는 제3의 시선**으로 씁니다. 그래서 먼저 테스트로 신뢰 경계를 세우고, AI가 낸 코드는 그 경계 안에서만 신뢰합니다. 직접 짜본 것만 제대로 의심할 수 있다는 걸 여러 번 겪었기 때문입니다.
